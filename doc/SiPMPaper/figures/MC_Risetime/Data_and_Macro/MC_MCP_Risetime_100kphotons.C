{
 	gROOT->SetBatch();
  gRandom->SetSeed(0);
  int NumPhotons=100000; //Number of Photons
  ...
  for(int i=820;i<1024;i++)
    h_avg->SetBinContent(i+1,exp(7.01884e-01-5.07216e-03*i));//Extrapolating the very-far tail of the pulse
...
h_MC->Fit("pol1","Q","",TenPercent_h_MC,SixtyPercent_h_MC);
 ...
 canv11->Print("Risetime_stat_Deadtime_Included_100k.root");//Risetime statistics (deadtime included)
