...
SiPMs are mounted on a printed circuit board (PCB) with the
circuit shown in Figure~\ref{fig:Circuit}. The high-pass filter is used to 
decrease the rise-time of signal pulses by restricting to high frequency
signals only, sometimes referred to as ``clipping''. As a consequence, 
the resulting signal pulse will be smaller in amplitude.
The SiPMs are mechanically attached to an
